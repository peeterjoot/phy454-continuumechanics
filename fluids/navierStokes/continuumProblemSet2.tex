...
\end{subequations}
%
With a steady state assumption we kill the \(\PDi{t}{u}\) term.
The x-component of the Laplacian and our non-linear inertial term on the LHS
is killed off with the help of \cref{eqn:continuumProblemSet2:100},
leaving just
%
\begin{subequations}
...
\end{subequations}
%
With \(\PD{z}{p} = \PD{y}{p} = 0\), we have \(\PD{x}{p} = dp/dx = -G\), so \cref{eqn:continuumProblemSet2:40a} is reduced to
%
\begin{equation}\label{eqn:continuumProblemSet2:40b}
0 = G + \mu \PDSq{y}{u}.
\end{equation}
%
Finally, since we have an assumption of no z-dependence (\(\PDi{z}{u} = 0\)) and from the incompressibility assumption \cref{eqn:continuumProblemSet2:100} (\(\PDi{x}{u} = 0\)), we have
%
...
For the velocity profile, integrating \cref{eqn:continuumProblemSet2:140} twice, we have
%
...
%\end{figure}
%
% continuumProblemSet2Fig1r2.cdf 
\imageFigure{../figures/phy454-continuumechanics/continuumProblemSet2Fig1r2}{Velocity profile for 1D constant pressure gradient steady state flow.}{fig:continuumProblemSet2:continuumProblemSet2Fig1}{0.2}
%
...
\end{equation}
%
Referring back to \cref{eqn:continuumProblemSet2:500} our velocities are
...
We see from \cref{eqn:continuumProblemSet2:600} the tangential components of our traction vectors are
%
...
We note that due to constant nature of the tangential component of the traction vector shown above, the force per unit width of the upper fluid acting on the lower fluid, is also given by \cref{eqn:continuumProblemSet2:740}.
} % end answer
%
...
We showed earlier that the Navier-Stokes equations for this Q3 case, where \(G\) is non-zero were given by \cref{eqn:continuumProblemSet2:400}, which restated is
...
% problemSetIIQ3PlotWithManipulate.cdf
