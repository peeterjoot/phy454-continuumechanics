...
\end{subequations}
%
Note that we can obtain \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:150} much more directly, if we know that is what we want to do.  Just operate on \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:5} with the gradient from the left right off the bat.  We find
%
...
%
It is this and \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:500} that we really have to solve.  Before moving on, let us write out all the non-boundary condition equations in coordinate form for the 2D case that we are interested in here.  We have
%
\begin{subequations}
...
\end{subequations}
%
Our solution has to satisfy these equations, as well as still satisfying the original Navier-Stokes system \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:40} that includes our gravitational term, and also has to satisfy both of our boundary value constraints \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:220}, \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:280}, and have \(u(x, 0) = v(x, 0) = 0\).  Wow, what a mess!  And this is all just the steady state problem.  Imagine adding time into the mix too!
%
...
I have expressed the curl as a wedge product, and not a cross product (either works since they related by a constant duality transformation).  With the wedge product the identity \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:641} has different signs.  That is
%
...
\end{subequations}
%
Having restated things with the \(\spacegrad \Bu^2\) term moved to the RHS, let us also now write out \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:760} and \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:780} in coordinate form (we want this for the 2D case).  This is
%
\begin{subequations}
...
\section{Now what?}
%
The strategy that I had thought to attempt to tackle this problem, when I had left it like \cref{eqn:inclinedFlowWithoutConstantHeightAssumption:599} was something along the following lines
\begin{itemize}
