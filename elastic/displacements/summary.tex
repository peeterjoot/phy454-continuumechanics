...
%
When a body is in static equilibrium \cref{eqn:continuumElasticityReview:570} reduces to just a simple force balance
%
...
\index{p-wave}
%
Operating on \cref{eqn:continuumElasticityReview:610} with the divergence operator, and writing \(\Theta = \spacegrad \cdot \Bu\), a quantity that was our relative change in volume in the diagonal strain basis, we were able to find this divergence obeys a wave equation
%
...
\index{s-wave}
%
Similarly, operating on \cref{eqn:continuumElasticityReview:610} with the curl operator, and writing \(\Bomega = \spacegrad \cross \Bu\), we were able to find this curl also obeys a wave equation
%
\begin{equation}\label{eqn:continuumElasticityReview:650}
\rho \PDSq{t}{\Bomega} = \mu \spacegrad^2 \Bomega.
\end{equation}
%
These we called S-waves.  We also noted that the (transverse) compression waves (P-waves) with speed
\begin{equation}\label{eqn:summary:20}
C_T = \sqrt{\mu/\rho},
\end{equation}
travelled faster than the (longitudinal) vorticity (S) waves with speed
\begin{equation}\label{eqn:summary:40}
C_L = \sqrt{(\lambda + 2 \mu)/\rho},
\end{equation}
and since \(\lambda > 0\) and \(\mu > 0\)
%
